...
5. For email verification, MFA, CAPTCHA, password, account-recovery, or passkey work, follow `references/recovery.md`. Use only an authorized mailbox and the newest message tied to a code request from the unchanged live session. Never persist a code or reuse a code from an earlier request.
